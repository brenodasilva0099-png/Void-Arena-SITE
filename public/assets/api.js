(function () {
  ...
  function userAvatar() { return FAV; }
  ...
  function setupUserPill(user = {}) { document.querySelectorAll('.va-user-pill').forEach((pill) => { pill.classList.add('is-clickable', 'is-compact', 'is-avatar-only'); pill.setAttribute('role', 'link'); pill.setAttribute('tabindex', '0'); pill.setAttribute('title', 'Abrir perfil'); pill.innerHTML = `<span class="va-user-pill-avatar"><img src="${escapeHtml(FAV)}" alt="Void Arena" /></span>`; const open = () => { window.location.href = '/pages/perfil.html'; }; pill.onclick = open; pill.onkeydown = (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(); } }; ensureTopbarTools(pill); }); setupNotificationButtons(); }
