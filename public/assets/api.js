(function () {
  ...
  function ensureTopbarTools(pill) { const topbar = pill.closest('.va-topbar'); if (!topbar) return pill.parentElement; let tools = topbar.querySelector(':scope > .va-topbar-tools'); if (!tools) { tools = document.createElement('div'); tools.className = 'va-topbar-tools'; topbar.appendChild(tools); } if (pill.parentElement !== tools) tools.appendChild(pill); return tools; }
  function openDiscordServer(event) { event?.preventDefault?.(); window.open('/api/discord/server/open', '_blank', 'noopener,noreferrer'); }
  function setupDiscordServerButtons() { document.querySelectorAll('.va-user-pill').forEach((pill) => { const tools = ensureTopbarTools(pill); let btn = tools.querySelector('.va-discord-pill'); if (!btn) { btn = document.createElement('button'); btn.className = 'va-discord-pill'; btn.type = 'button'; btn.title = 'Abrir servidor do Discord'; btn.setAttribute('aria-label', 'Abrir servidor do Discord'); btn.addEventListener('click', openDiscordServer); pill.insertAdjacentElement('afterend', btn); } btn.innerHTML = '<span>☄️</span>'; }); }
  ...
  ensureLegalFooter();
  ensureFavicon();
  Object.assign(VA, { request, escapeHtml, formatDate, loadMe, loadBrand, applyBrand, bootLayout, profileUsername, userAvatar, openNotifications, loadAccess, ensureLegalFooter, openDiscordServer });
}());
