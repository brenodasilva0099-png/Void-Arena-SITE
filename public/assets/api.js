(function () {
  ...
  function ensureCleanupStyles() {
    ...
    link.rel = 'stylesheet';
    link.href = '/css/site-cleanup.css?v=4';
  ...
  function applyBrand(brand = {}) { const icon = brand.icon || brand.fallbackIcon || '/assets/hollow-nexus.png'; const name = brand.name || 'Hollow Nexus'; document.querySelectorAll('[data-server-name]').forEach((el) => { el.textContent = name; }); document.querySelectorAll('[data-server-icon]').forEach((img) => { img.src = icon; img.alt = `Ícone ${name}`; }); let favicon = document.querySelector('link[rel="icon"]') || document.querySelector('link[rel="shortcut icon"]'); if (!favicon) { favicon = document.createElement('link'); favicon.rel = 'icon'; document.head.appendChild(favicon); } favicon.href = icon; }
  ...
  async function bootLayout(activeKey = '') { ensureCleanupStyles(); ensureExtraNavLinks(); const user = await loadMe(); setupUserPill(user); loadBrand().then(applyBrand).catch(() => applyBrand({})); document.querySelectorAll('[data-user-name]').forEach((el) => { el.textContent = profileUsername(user); }); document.querySelectorAll('[data-nav-key]').forEach((el) => { el.classList.toggle('active', el.getAttribute('data-nav-key') === activeKey); }); loadAccess(activeKey); return { user, brand: null }; }

  ensureCleanupStyles();
  Object.assign(VA, { request, escapeHtml, formatDate, loadMe, loadBrand, applyBrand, bootLayout, profileUsername, userAvatar, openNotifications, loadAccess });
}());
