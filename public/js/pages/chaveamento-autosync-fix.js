(function () {
  ...
  const DEFAULT_MATCH_CATEGORY_ID = '1523133579570184194';
  const categoryField = document.querySelector('[name="discordMatchCategoryId"]');
  if (categoryField && !String(categoryField.value || '').trim()) categoryField.value = DEFAULT_MATCH_CATEGORY_ID;
  ...
  function teamInitials(team, fallback = '?') { return esc(String(team?.tag || team?.name || fallback || '?').slice(0, 2).toUpperCase()); }
  function teamLogo(team, fallback = '?') { return team?.logo ? `<img src="${esc(team.logo)}" alt="${esc(team.name || team.tag || fallback)}" />` : `<span>${teamInitials(team, fallback)}</span>`; }
  function ensureGroupStyles() {
    if (document.getElementById('generatedGroupsStyles')) return;
    const style = document.createElement('style');
    style.id = 'generatedGroupsStyles';
    style.textContent = `.va-generated-groups{margin-top:16px}.va-groups-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}.va-group-card{border:1px solid rgba(139,92,246,.28);border-radius:18px;padding:14px;background:rgba(255,255,255,.04)}.va-group-card h3{margin:0 0 10px;color:#67e8f9}.va-group-team{display:grid;grid-template-columns:38px 1fr auto;gap:10px;align-items:center;padding:9px 10px;border-radius:12px;background:rgba(2,6,23,.45);margin-top:8px}.va-group-logo{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;background:rgba(139,92,246,.25);border:1px solid rgba(34,211,238,.25);overflow:hidden}.va-group-logo img{width:100%;height:100%;object-fit:cover}.va-group-logo span{font-size:11px;color:#f8f7ff;font-weight:1000}.va-group-team strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.va-group-team small{color:#a5f3fc;font-weight:900}`;
