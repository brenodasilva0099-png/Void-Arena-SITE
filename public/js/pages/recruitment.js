(function () {
  ...
  function openPublicPlayer(player = {}) {
    ...
    overlay.innerHTML = `<div class="va-modal-card va-public-profile-card va-player-public-card"><button class="va-modal-close va-floating-close" data-directory-close type="button">×</button><div class="va-public-banner" style="${banner ? `background-image:linear-gradient(180deg,rgba(0,0,0,.05),rgba(0,0,0,.72)),url('${esc(banner)}')` : ''}"></div><div class="va-public-head"><div class="va-profile-page-avatar va-public-avatar">${player.avatar ? `<img src="${esc(player.avatar)}" alt="Avatar" />` : esc(name.slice(0, 1).toUpperCase())}</div><div><p class="va-eyebrow">Perfil público do jogador</p><h2>${esc(name)}</h2><p class="va-muted">${esc([profile.primaryPosition || player.primaryPosition, profile.secondaryPosition, profile.region || profile.competitiveRegion || player.region, profile.country || player.country].filter(Boolean).join(' • ') || player.statusLabel || 'Jogador')}</p><div class="va-kpi-row"><span class="va-badge ${player.status === 'free' ? 'ok' : ''}">${esc(player.statusLabel || 'Jogador')}</span><span class="va-badge">${esc(player.rosterRole || 'Perfil público')}</span>${player.teamName ? `<span class="va-badge">${esc(player.teamName)}${player.teamTag ? ` • ${esc(player.teamTag)}` : ''}</span>` : ''}</div></div></div><div class="va-public-section"><h3>Times vinculados</h3>${teams.length ? `<div class="va-player-team-list">${teams.map((team) => `<div><strong>${esc(team.name)}</strong><small>${esc(team.tag || 'Sem tag')}</small></div>`).join('')}</div>` : '<p class="va-muted">Jogador livre para recrutamento.</p>'}</div><div class="va-public-section"><h3>Conexões públicas</h3>${connectionCards(player.socials)}</div>${profile.bio ? `<div class="va-public-section"><h3>Bio</h3><p class="va-muted">${esc(profile.bio)}</p></div>` : ''}</div>`;
