(function () {
  ...
  async function enhance() {
    ...
    document.querySelectorAll('[data-team-id]').forEach((card) => {
      ...
      if ((!team?.canManage && !team?.canDelete) || card.querySelector('[data-manage-team-actions]')) return;
      const actions = document.createElement('div');
      actions.className = 'va-actions team-actions';
      actions.setAttribute('data-manage-team-actions', '1');
      actions.innerHTML = `${team.canManage ? '<button class="va-btn mini" data-team-action="edit" type="button">Editar</button><button class="va-btn mini secondary" data-team-action="transfer" type="button">Transferir capitão</button>' : ''}${team.canDelete ? '<button class="va-btn danger mini" data-team-action="delete" type="button">Excluir</button>' : ''}`;
      actions.querySelector('[data-team-action="edit"]')?.addEventListener('click', (event) => { event.stopPropagation(); openEdit(team); });
      actions.querySelector('[data-team-action="transfer"]')?.addEventListener('click', (event) => { event.stopPropagation(); openTransfer(team); });
      actions.querySelector('[data-team-action="delete"]')?.addEventListener('click', (event) => { event.stopPropagation(); removeTeam(team); });
