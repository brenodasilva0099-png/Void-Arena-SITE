(async function () {
  ...
  function openSettings(eventId) { const event = events.find((item) => String(item.id) === String(eventId)); if (!event || !settingsModal || !settingsForm) return; settingsTitle.textContent = `Editar ${event.title || event.name || 'evento'}`; settingsForm.elements.eventId.value = event.id; settingsForm.elements.title.value = event.title || event.name || ''; settingsForm.elements.startAt.value = dtToInput(event.startAt || ''); settingsForm.elements.status.value = event.status || 'open'; settingsForm.elements.matchFormat.value = event.matchFormat || 'MD1'; settingsForm.elements.structure.value = event.structure || 'single_elimination'; settingsForm.elements.mode.value = event.mode || ''; settingsForm.elements.teamLimit.value = String(event.teamLimit || 16); settingsForm.elements.reward.value = event.reward || event.prize || ''; settingsForm.elements.entryFee.value = event.entryFee || event.registrationFee || ''; settingsForm.elements.isFree.checked = event.isFree === true || !String(event.entryFee || event.registrationFee || '').trim(); settingsForm.elements.paymentInstructions.value = event.paymentInstructions || ''; settingsForm.elements.description.value = event.description || ''; setSettingsStatus('Ao salvar, as DMs antigas desse evento serão editadas em vez de mandar nova notificação.'); settingsModal.hidden = false; }
  function closeSettings() { if (settingsModal) settingsModal.hidden = true; }

  async function announceEvent(eventId) {
    const event = events.find((item) => String(item.id) === String(eventId));
    if (!event) return;
    const message = window.prompt('Mensagem do anúncio para os capitães:', `Atualização sobre ${event.title || event.name}:`);
    if (message === null) return;
    setStatus('Enviando anúncio aos capitães...');
    try {
      const data = await VoidArena.request(`/api/events/${encodeURIComponent(eventId)}/announce`, { method: 'POST', body: JSON.stringify({ message }) });
      const sent = data.notice?.sent || 0;
      setStatus(`Anúncio enviado. DMs novas: ${sent}.`, 'ok');
    } catch (error) { setStatus(`Erro ao anunciar: ${error.message}`, 'err'); }
  }

  async function render() { const data = await fetchJson('/api/events'); events = (data.events || []).sort((a, b) => (a.status === 'open' ? -1 : 1) - (b.status === 'open' ? -1 : 1)); teams = await loadTeams(); list.innerHTML = events.length ? events.map(eventCard).join('') : '<div class="va-item">Nenhum evento cadastrado no momento.</div>'; list.querySelectorAll('[data-register-event]').forEach((btn) => btn.addEventListener('click', () => openRegister(btn.dataset.registerEvent))); list.querySelectorAll('[data-config-event]').forEach((btn) => btn.addEventListener('click', () => openSettings(btn.dataset.configEvent))); list.querySelectorAll('[data-announce-event]').forEach((btn) => btn.addEventListener('click', () => announceEvent(btn.dataset.announceEvent))); setStatus(events.length ? `Eventos carregados: ${events.length}.` : 'Nenhum evento disponível agora.', events.length ? 'ok' : ''); }
