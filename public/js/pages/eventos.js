(async function () {
  ...
  const registeredManager = document.getElementById('eventRegisteredTeamsManager');
  let events = [];
  let teams = [];
  ...
  function teamLogoUrl(team = null) { const id = String(team?.id || '').trim(); if (id) return `/api/team-logo/${encodeURIComponent(id)}?v=${encodeURIComponent(team?.updatedAt || team?.logoUpdatedAt || id)}`; return String(team?.logo || team?.logoUrl || '').trim(); }
  function registrationTeam(registration = {}) { const team = registration.team || teamById(registration.teamId); if (team) return team; const name = registration.teamName || registration.name || 'Time inscrito'; return { id: registration.teamId || '', name, tag: registration.teamTag || name.slice(0, 3), logo: registration.teamLogo || registration.teamLogoUrl || '' }; }
  function initialsFromTeam(team = null) { return esc((team?.tag || team?.name || 'TM').slice(0, 2).toUpperCase()); }
  function teamLogoInner(team = null) { if (!team) return '<span>TM</span>'; const src = teamLogoUrl(team); if (src) return `<img src="${esc(src)}" alt="${esc(team.name || team.tag || 'Time')}" loading="lazy" />`; return `<span>${initialsFromTeam(team)}</span>`; }
  ...
  function openEventTeamProfile(teamId = '') {
    ...
    const logoSrc = teamLogoUrl(team);
    overlay.innerHTML = `<div class="va-modal-card va-public-profile-card va-team-public-card"><button class="va-modal-close va-floating-close" type="button" data-event-team-close>×</button><div class="va-public-banner va-team-public-banner">${logoSrc ? `<img src="${esc(logoSrc)}" alt="" />` : ''}</div><div class="va-public-head"><div class="va-public-team-logo">${logoSrc ? `<img src="${esc(logoSrc)}" alt="Logo ${esc(team.name)}" />` : esc((team.tag || team.name || 'T').slice(0, 2).toUpperCase())}</div><div><p class="va-eyebrow">Perfil público do time</p><h2>${esc(team.name || 'Time')} ${team.tag ? `<span class="va-muted">(${esc(team.tag)})</span>` : ''}</h2><p class="va-muted">Capitão: ${esc(team.captainName || team.ownerName || 'não definido')}</p><div class="va-kpi-row"><span class="va-badge">Titulares ${players.length}</span><span class="va-badge">Reservas ${reserves.length}</span><span class="va-badge">${esc(team.tag || 'TIME')}</span></div></div></div><div class="va-public-section"><h3>Titulares</h3><div class="va-team-roster">${players.map((p) => publicPlayerRow(p, '⚽')).join('') || '<div class="va-player-row">Nenhum titular detalhado.</div>'}</div></div><div class="va-public-section"><h3>Reservas</h3><div class="va-team-roster">${reserves.map((p) => publicPlayerRow(p, '🧤')).join('') || '<div class="va-player-row">Nenhum reserva.</div>'}</div></div></div>`;
