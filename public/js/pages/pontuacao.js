(function () {
  const RULES = [
    ['3', 'Vitória', 'Base principal da pontuação'],
    ['1', 'Empate', 'Usado em grupos e jogos empatados'],
    ['0', 'Derrota', 'Não soma pontos'],
    ['+0,5', 'Por gol marcado', 'Bônus ofensivo sem desbalancear'],
    ['+1', 'Clean sheet', 'Bônus por não sofrer gol'],
    ['+2', 'Participação', 'Time validado no evento'],
    ['+30', 'Campeão', 'Bônus máximo de colocação'],
    ['x0.5–x2.5', 'Peso do evento', 'Scrim, pequeno, oficial, principal ou elite']
  ];
  ...
  function normalizeResult(result = {}) {
    ...
    if (!goalsA && !goalsB && games.length) {
      games.forEach((game) => {
        ...
        goalsA += num(game.finalScoreA ?? game.scoreA);
        goalsB += num(game.finalScoreB ?? game.scoreB);
      });
    }

    if (!goalsA && !goalsB) {
      goalsA = num(result.finalScoreA);
      goalsB = num(result.finalScoreB);
    }
    ...
    return {
      ...
      goalsA,
      goalsB,
      scoreA: num(result.seriesScoreA ?? result.finalScoreA),
      scoreB: num(result.seriesScoreB ?? result.finalScoreB),
  ...
  function teamName(team = {}, fallback = 'Time') {
    return team.name || team.tag || fallback;
  }

  function renderRules() {
    rulesEl.innerHTML = RULES.map(([value, title, text]) => '<article class="va-scoring-rule-card"><strong>' + esc(value) + '</strong><h3>' + esc(title) + '</h3><p>' + esc(text) + '</p></article>').join('');
  }

  function renderEvents(events = []) {
    const selected = eventFilterEl.value || '';
    eventFilterEl.innerHTML = '<option value="">Todos os eventos</option>' + events.map((event) => '<option value="' + esc(event.id) + '">' + esc(event.name || event.title || 'Evento') + '</option>').join('');
    eventFilterEl.value = selected;
  }
  ...
  async function load() {
    statusEl.textContent = 'Carregando regras e validações...';
    ...
    renderRules();
    renderSummary(results, selectedEventId);
    renderValidation(results, selectedEventId);
  ...
  VoidArena.bootLayout('pontuacao').then(load).catch((error) => {
