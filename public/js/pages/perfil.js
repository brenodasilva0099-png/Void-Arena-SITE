(function () {
  ...
  function icon(key) { return { steam: '🎮', xbox: '🟢', tiktok: '🎵', youtube: '▶️', twitter: '𝕏', spotify: '🎧', riot: '🔥', ea: '🎯', psn: '🎮' }[key] || '🔗'; }
  function label(key) { return { steam: 'Steam', xbox: 'Xbox', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X', spotify: 'Spotify', riot: 'Riot ID', ea: 'EA ID', psn: 'PSN' }[key] || key; }
  function renderConnections(socials = {}) {
    if (!connectionsEl) return;
    const entries = Object.entries(socials || {}).filter(([, v]) => String(v || '').trim());
    connectionsEl.innerHTML = entries.length ? entries.map(([key, val]) => `<div class="va-connection-card"><strong>${icon(key)} ${label(key)}</strong><span>${esc(val)}</span></div>`).join('') : '<div class="va-muted">Nenhuma conexão pública cadastrada.</div>';
  }
  ...
  function fill(payload = {}) {
    ...
    value('steam', socials.steam || profile.steamId || ''); value('xboxGamertag', socials.xbox || profile.xboxGamertag || ''); value('tiktok', socials.tiktok || ''); value('youtube', socials.youtube || ''); value('twitter', socials.twitter || ''); value('spotify', socials.spotify || ''); value('riot', socials.riot || ''); value('ea', socials.ea || ''); value('psn', socials.psn || '');
    const displayName = profile.username || user.name || 'Perfil';
    ...
    renderConnections(socials); renderTeam(payload.currentTeam || null); renderStats(payload.stats || {});
  }
  ...
  async function save() {
    ...
    const body = { profile: { username: read('username'), realName: read('realName'), country: read('country'), region: read('region'), timezone: read('timezone'), primaryPosition: read('primaryPosition'), secondaryPosition: read('secondaryPosition'), bio: read('bio'), banner: read('banner'), steamId: read('steam'), xboxGamertag: read('xboxGamertag') }, socials: { steam: read('steam'), xbox: read('xboxGamertag'), tiktok: read('tiktok'), youtube: read('youtube'), twitter: read('twitter'), spotify: read('spotify'), riot: read('riot'), ea: read('ea'), psn: read('psn') } };
    const data = await VoidArena.request('/api/me/profile-v2', { method: 'PUT', body: JSON.stringify(body) });
    fill(data); setStatus('Perfil salvo com sucesso.', 'ok');
  }
