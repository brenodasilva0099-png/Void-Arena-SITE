(function () {
  ...
  function notificationOverlay() {
    ...
    if (!overlay) {
      ...
      overlay.addEventListener('click', (event) => {
        if (event.target === overlay || event.target.closest('[data-notification-close]')) overlay.hidden = true;
      });
  ...
  async function actionNotification(id, action) {
    await request(`/api/notifications/${encodeURIComponent(id)}/action`, {
      method: 'POST',
  ...
  async function openNotifications() {
    ...
    try {
      ...
      overlay.querySelectorAll('[data-notification-action]').forEach((btn) => {
        btn.addEventListener('click', () => actionNotification(btn.dataset.notificationId, btn.dataset.notificationAction).catch((error) => alert(error.message)));
      });
    } catch (error) {
  ...
  function showAccessDenied(pageKey = '') {
    ...
    if (!main) return;
    main.querySelectorAll('.va-grid, section:not(.va-access-denied), article:not(.va-access-denied)').forEach((el) => {
      if (!el.closest('.va-topbar')) el.remove();
    });
  ...
  function applyAccessMap(access = {}, activeKey = '') {
    ...
    document.querySelectorAll('[data-nav-key]').forEach((link) => {
      ...
      if (Object.prototype.hasOwnProperty.call(access, key) && access[key] === false) link.hidden = true;
      else link.hidden = false;
    });
    if (activeKey && Object.prototype.hasOwnProperty.call(access, activeKey) && access[activeKey] === false) {
      showAccessDenied(activeKey);
      return false;
    }
    return true;
  }
  ...
  async function loadAccess(activeKey = '') {
    try {
      ...
      return { success: true, degraded: true, access: {} };
    }
  }
