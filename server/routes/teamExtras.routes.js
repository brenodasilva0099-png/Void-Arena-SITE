...
function enrichTeam(team = {}, users = [], viewer = null) {
  ...
  const mapPlayer = (entry, type, index, account = '') => {
    ...
    return { id: user?.id || (typeof entry === 'object' ? entry.id || '' : ''), name: user ? userDisplay(user) : clean(name || `Jogador ${index + 1}`), account: clean(rawDiscord), discordId: user?.discordId || discordId || '', avatar: user?.avatar || (typeof entry === 'object' ? entry.avatar || '' : ''), profile: user?.profile || (typeof entry === 'object' ? entry.profile || {} : {}), type, isCaptain: Boolean(owner && user && String(owner.id) === String(user.id)) };
  };
  ...
  return { ...safe, ownerName: owner ? userDisplay(owner) : (safe.ownerName || safe.captainName || 'não definido'), ownerAvatar: owner?.avatar || safe.ownerAvatar || '', captainName: owner ? userDisplay(owner) : (safe.captainName || 'não definido'), captainDiscordId: owner?.discordId || safe.captainDiscordId || '', playerDetails, reserveDetails, canManage: canManageTeam(viewer, team) };
}

function findUser(users = [], { userId = '', discordId = '' } = {}) {
  const safeUserId = clean(userId); const safeDiscordId = clean(discordId);
  return users.find((user) => (safeUserId && String(user.id || '') === safeUserId) || (safeDiscordId && String(user.discordId || '') === safeDiscordId)) || null;
}
...
function registerTeamExtrasRoutes(app) {
  ...
  app.post('/api/teams/:teamId/transfer-captain', requireSession, async (req, res) => {
    try {
      ...
      const saved = await storage.saveTeam({ ...team, ownerUserId: target.id, ownerName: userDisplay(target), ownerAvatar: target.avatar || '', captainName: userDisplay(target), captainDiscordId: target.discordId || '', updatedAt: new Date().toISOString() });
