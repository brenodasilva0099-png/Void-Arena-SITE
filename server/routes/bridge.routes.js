...
const BRIDGES = {
  chat: {
    ...
    placeholder: 'Enviar mensagem para o chat do site... Use @ para marcar membros/cargos.'
  },
  estatisticas: {
    title: 'Estatísticas',
    ...
    placeholder: 'Enviar mensagem para o canal de estatísticas... Use @ para marcar membros/cargos.'
  },
  scrims: {
    title: 'Scrims',
    siteChannelId: 'scrims-main',
    readSettings: () => storage.readChatBridgeSettings().then((settings) => ({ ...settings, siteChannelId: 'scrims-main', discordChannelId: settings.scrimsDiscordChannelId || '' })),
    writeSettings: async (settings) => {
      const current = await storage.readChatBridgeSettings().catch(() => ({}));
      const saved = await storage.writeChatBridgeSettings({
        ...current,
        enabled: current.enabled,
        siteChannelId: current.siteChannelId || 'site-main',
        discordChannelId: current.discordChannelId || '',
        scrimsDiscordChannelId: settings.discordChannelId || ''
      });
      return { ...saved, siteChannelId: 'scrims-main', discordChannelId: settings.discordChannelId || '' };
    },
    placeholder: 'Enviar mensagem de scrim/contato entre times... Use @ para marcar membros/cargos.'
  }
};
...
function publicMessage(message = {}) {
  return {
    ...
    authorName: message.authorName || 'Void Arena',
...
async function readChannels() {
  const data = await callBot('/internal/discord/channels', { method: 'GET' }).catch((error) => ({ success: false, channels: [], message: error.message }));
  return { channels: Array.isArray(data.channels) ? data.channels : [], channelMessage: data.message || data.internalError || '' };
}
...
function registerBridgeRoutes(app) {
  app.get('/api/bridge/:key/state', requireSession, async (req, res) => {
    try {
      ...
      const [settings, messages, channelsData] = await Promise.all([
        bridge.readSettings().catch(() => ({ enabled: false, siteChannelId: bridge.siteChannelId, discordChannelId: '' })),
        storage.readChatMessages({ channelId: bridge.siteChannelId, limit: 80 }).catch(() => []),
        readChannels()
      ]);
      return res.json({
        success: true,
        bridge: { key: req.params.key, title: bridge.title, placeholder: bridge.placeholder },
        settings: { enabled: Boolean(settings.enabled), siteChannelId: bridge.siteChannelId, discordChannelId: settings.discordChannelId || '' },
  ...
  app.post('/api/bridge/:key/messages', requireSession, async (req, res) => {
    try {
      ...
      if (settings.discordChannelId) {
        discord = await callBot('/internal/discord/send-message', {
          method: 'POST',
          body: JSON.stringify({ discordChannelId: settings.discordChannelId, content: `**${user?.profile?.username || user?.name || 'Void Arena'}:** ${content}`, allowedMentions: { parse: ['users', 'roles'] } })
