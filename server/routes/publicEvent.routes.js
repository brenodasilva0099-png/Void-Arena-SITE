...
const { SEASON_ONE, isVisibleCompetition, publicSeason, withSeasonCompetitions } = require('../services/season.service');
...
function feeLabel(event = {}) {
  ...
  if (fee) return fee;
  return String(event.status || '').toLowerCase() === 'upcoming' ? 'A definir' : 'F2P';
}
...
function registerPublicEventRoutes(app) {
  ...
  app.get('/api/events', async (_req, res) => { const [events, teams] = await Promise.all([storage.readEvents().catch(() => []), storage.readTeams().catch(() => [])]); const competitions = withSeasonCompetitions(events); return res.json({ success: true, season: publicSeason(competitions), events: competitions.map((event) => safeEvent(event, teams)) }); });
