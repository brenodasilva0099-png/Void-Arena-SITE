...
const TARGETS = ['dashboard.html','competicoes.html','clubes.html','atletas.html','rankings.html','resultados.html'];
...
function inject(fileName) {
  ...
  if (!html.includes('/css/hollow-v2-runtime.css')) html = html.includes('</head>') ? html.replace('</head>', `${CSS}\n</head>`) : `${CSS}\n${html}`;
  if (!html.includes('/js/core/hollow-v2-runtime.js')) html = html.includes('</body>') ? html.replace('</body>', `${JS}\n</body>`) : `${html}\n${JS}`;
  write(file, html);
  return true;
}
