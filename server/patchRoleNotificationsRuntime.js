...
function addAppRoutes() {
  ...
  if (src.includes("ROLE_NOTIFY_HISTORY_CHANNEL")) return;
  ...
  const route = [
    ...
    "        byDiscordId.set(discordId, { id: member.id || discordId, discordId, name: member.name || member.username || discordId, username: member.username || '', avatar: member.avatar || '', guildName: member.guildName || '', roles: Array.isArray(member.roles) ? member.roles : [], registered: false });",
    "      });",
    "      (users || []).forEach((user) => {",
    "        const discordId = String(user.discordId || '').trim();",
    "        if (!/^\\d{16,22}$/.test(discordId)) return;",
    "        const current = byDiscordId.get(discordId) || {};",
    "        byDiscordId.set(discordId, { ...current, id: user.id || current.id || discordId, siteUserId: user.id || '', discordId, name: rnName(user) || current.name || discordId, username: user?.profile?.username || current.username || '', avatar: user.avatar || current.avatar || '', roles: current.roles || [], registered: true });",
    "      });",
...
function patchConfigJs() {
  ...
  if (!js.includes('function renderDmPlayerOptions')) {
    ...
    js = js.replace('  function targetLine(target = {}) {', helpers + '  function targetLine(target = {}) {');
    jsChanged = true;
  }

  if (!js.includes("/api/admin/role-notifications/players")) {
    js = js.replace("const [rolesData, historyData] = await Promise.all([", "const [rolesData, historyData, playersData] = await Promise.all([");
    js = js.replace("VoidArena.request('/api/admin/role-notifications/history', { timeoutMs: 12000 }).catch((error) => ({ success: false, campaigns: [], message: error.message }))", "VoidArena.request('/api/admin/role-notifications/history', { timeoutMs: 12000 }).catch((error) => ({ success: false, campaigns: [], message: error.message })),\n        VoidArena.request('/api/admin/role-notifications/players', { timeoutMs: 16000 }).catch((error) => ({ success: false, players: [], message: error.message }))");
    js = js.replace("      renderRoleHistory(historyData.campaigns || []);", "      renderRoleHistory(historyData.campaigns || []);\n      renderDmPlayerOptions(playersData.players || []);");
    js = js.replace("      status(roleNotificationStatus, `${rolesData.roles?.length || 0} cargo(s) carregado(s). Histórico: ${historyData.campaigns?.length || 0} envio(s).`, 'ok');", "      status(roleNotificationStatus, String(rolesData.roles?.length || 0) + ' cargo(s) carregado(s). Histórico: ' + String(historyData.campaigns?.length || 0) + ' envio(s). Jogadores: ' + String(playersData.players?.length || 0) + '.', 'ok');");
    jsChanged = true;
  }
