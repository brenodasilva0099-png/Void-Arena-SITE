...
const BUILD = '2026-07-28-club-leadership-v8';
...
function hero(title, text, icon, kicker = 'Hollow Nexus League') {
  ...
  return `<section class="frm-page-hero hnl-section-hero" data-hnl-scene="${scene}"><div><span class="hnl-section-kicker">${kicker}</span><h1>${title}</h1><p>${text}</p></div><div class="hnl-hero-icon hnl-section-scene" data-scene="${scene}" aria-hidden="true"><span class="hnl-scene-stage"><span class="hnl-scene-orbit"></span><span class="hnl-scene-core"></span><span class="hnl-scene-node one"></span><span class="hnl-scene-node two"></span><span class="hnl-scene-node three"></span><span class="hnl-scene-trace"></span><span class="hnl-scene-detail"></span><strong>${heroSceneLabel(scene)}</strong></span></div></section>`;
}
function rematchHomeHero() {
  return `<section class="frm-page-hero hnl-rematch-hero">
    <div class="hnl-rematch-field" aria-hidden="true">
      <span class="hnl-rematch-goal left"></span>
      <span class="hnl-rematch-goal right"></span>
      <span class="hnl-rematch-penalty-arc left"></span>
      <span class="hnl-rematch-penalty-arc right"></span>
      <span class="hnl-rematch-penalty-spot left"></span>
      <span class="hnl-rematch-penalty-spot right"></span>
      <span class="hnl-rematch-corner tl"></span>
      <span class="hnl-rematch-corner tr"></span>
      <span class="hnl-rematch-corner bl"></span>
      <span class="hnl-rematch-corner br"></span>
    ...
    <div class="hnl-rematch-copy">
      ...
      <h1>the HOLLOW NEXUS <span class="frm-accent">LEAGUE</span></h1>
      <p>Liga comunitária de Rematch para clubes, jogadores, competições, calendário e eventos.</p>
      <div class="hnl-rematch-tags" aria-label="Destaques da liga">
        <span>Liga comunitária</span><span>Clubes e jogadores</span><span>Temporadas de Rematch</span>
      </div>
    </div>
  </section>`;
}
...
const clubRegionOptions = ['Brasil', 'Sudeste', 'Sudoeste', 'Sul', 'Norte', 'Nordeste', 'Centro-Oeste', 'América do Sul', 'LATAM Sul', 'LATAM Norte', 'América do Norte', 'Europa', 'Ásia', 'Oceania', 'Outro']
  .map((region) => `<option value="${region}">${region}</option>`)
  .join('');

const competitionSeasonBody = `<section class="hnl-season-banner" id="currentSeasonCard" aria-labelledby="currentSeasonTitle">
  <div class="hnl-season-visual" aria-hidden="true"><span class="hnl-season-ring one"></span><span class="hnl-season-ring two"></span><span class="hnl-season-ball">⚽</span><span class="hnl-season-line"></span></div>
  <div class="hnl-season-copy">
    <span class="hnl-season-eyebrow">Novo ciclo competitivo</span>
    <h2 id="currentSeasonTitle"><span id="currentSeasonTitleText">Temporada 1</span><strong id="currentSeasonName">Hollow Nexus T1</strong></h2>
    <p id="currentSeasonDescription">Primeiro ciclo oficial da liga. Os próximos campeonatos serão reunidos nesta temporada, com calendário, súmulas, resultados e rankings conectados.</p>
    <div class="hnl-season-chips"><span id="currentSeasonStatus">Em preparação</span><span>Calendário em definição</span></div>
  </div>
  <div class="hnl-season-numbers" aria-label="Resumo da temporada">
    <div><strong id="seasonCompetitionCount">0</strong><span>Campeonatos</span></div>
...
const pages = {
  ...
  'times.html': '',
  'cadastrar-clube.html': shell({ title: 'Cadastrar Clube', tab: 'clubes', href: '/pages/cadastrar-clube.html', module: 'create-club', heroHtml: hero('Cadastrar Clube', 'Crie um clube e escolha Diretor e Capitão entre usuários reais vinculados ao Discord.', '+'), body: `<section class="hnl-card"><div id="createClubStatus"></div><form id="createClubForm" class="hnl-form-grid"><div class="hnl-field"><label>Nome do clube</label><input class="hnl-input" name="name" required></div><div class="hnl-field"><label>Tag</label><input class="hnl-input" name="tag" maxlength="12" required></div><div class="hnl-field full"><div class="hnl-console-head"><div><h2>Liderança do clube</h2><p class="frm-muted">Selecione contas reais cadastradas no site e reconhecidas pelo Discord. Depois da criação, somente administrador ou criador original poderá trocar estes responsáveis.</p></div></div></div><div class="hnl-field"><label>Diretor / dono</label><select class="hnl-select" id="createClubDirector" name="directorUserId" required><option value="">Carregando usuários...</option></select></div><div class="hnl-field"><label>Capitão</label><select class="hnl-select" id="createClubCaptain" name="captainUserId" required><option value="">Carregando usuários...</option></select></div><div class="hnl-field"><label>Região</label><select class="hnl-select" name="region" required><option value="">Selecionar região</option>${clubRegionOptions}</select></div><div class="hnl-field"><label>Logo do clube</label><input class="hnl-input" name="logo" placeholder="Cole uma URL, escolha, arraste ou cole uma imagem"><small class="frm-muted">Aceita URL direta, arquivo PNG/JPG/WEBP, arrastar e soltar ou Ctrl+V.</small></div><div class="hnl-field full"><label>Descrição</label><textarea class="hnl-textarea" name="description"></textarea></div><div class="hnl-field full"><h2>Conexões públicas</h2><p class="frm-muted">Esses links aparecerão com as logos dos aplicativos no perfil público.</p></div><div class="hnl-field"><label>Discord</label><input class="hnl-input" name="socialDiscord" placeholder="https://discord.gg/..."></div><div class="hnl-field"><label>Instagram</label><input class="hnl-input" name="socialInstagram" placeholder="https://instagram.com/..."></div><div class="hnl-field"><label>X / Twitter</label><input class="hnl-input" name="socialTwitter" placeholder="https://x.com/..."></div><div class="hnl-field"><label>TikTok</label><input class="hnl-input" name="socialTiktok" placeholder="https://tiktok.com/@..."></div><div class="hnl-field"><label>YouTube</label><input class="hnl-input" name="socialYoutube" placeholder="https://youtube.com/@..."></div><div class="hnl-field"><label>Twitch</label><input class="hnl-input" name="socialTwitch" placeholder="https://twitch.tv/..."></div><div class="hnl-field full"><label>Site</label><input class="hnl-input" name="socialWebsite" placeholder="https://..."></div><div class="hnl-actions full"><button class="hnl-btn primary">Criar clube</button></div></form></section>`, extraScripts: `<script src="/js/pages/team-logo-upload.js?v=${BUILD}"></script>` }),
  ...
  'perfil-clube.html': shell({ title: 'Perfil do Clube', tab: 'clubes', href: '/pages/clubes.html', module: 'club-profile', heroHtml: hero('Perfil Público do Clube', 'Direção, capitão, elenco, conexões e edição segura da liderança.', '◈'), body: '<div id="clubPublicProfile"></div>', extraScripts: `<script src="/js/pages/team-logo-upload.js?v=${BUILD}"></script>` }),
